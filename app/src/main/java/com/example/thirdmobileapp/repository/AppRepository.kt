...
class AppRepository {
    companion object {
        ...
        fun getInstance(): AppRepository {
            ...
            return INSTANCE ?:
            throw IllegalArgumentException("Репозиторий не инициализирован")
        }
    }
    ...
    fun setCurrentGroup(position: Int) {
        if (listOfGroup.value == null || position < 0 || (listOfGroup.value?.items?.size!!<=position))
            return
    ...
    fun getStudentPosition(student: Student): Int = listOfFaculty.value?.items?.indexOfFirst {
        it.id == student.id
    } ?: -1

    fun setCurrentStudent(position: Int) {
        if (listOfStudent.value == null || position < 0 || (listOfStudent.value?.items?.size!! <= position))
            return
    ...
    fun deleteStudent(student: Student){
        val listTpm = listOfStudent.value ?: ListOfStudent()
        if (listTpm.items.remove(student))
            listOfStudent.postValue(listTpm)
        setCurrentStudent(0)
    }
    ...
    fun getGroupStudent(groupID : UUID) =
        listOfStudent.value?.items?.filter {
            it.groupID == groupID
        }?.sortedBy {
            it.shortName
        }  ?: listOf()
